...
SlippyMap::SlippyMap(QObject *parent)
    : QObject(parent), width(400), height(300), zoom(15),
      latitude(37.54866), longitude(127.070501)
{
...
{    
    int x,y;
  //      printf("width : %d, height : %d\n",m_tilesRect.width(),m_tilesRect.height());
  //      printf("left : %d, top : %d\n",m_tilesRect.left(),m_tilesRect.top());
    for ( x = 0; x <= m_tilesRect.width(); ++x)
    {   for ( y = 0; y <= m_tilesRect.height(); ++y) {
    ...
    imgWidth = m_tilesRect.right() - m_tilesRect.left();
}

void SlippyMap::pan(const QPoint &delta)
{
    QPointF dx = QPointF(delta) / qreal(tdim);
    QPointF Coord = tileForCoordinate(latitude, longitude, zoom);
    QPointF center = Coord - dx;
